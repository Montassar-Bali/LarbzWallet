...
const ondoMarketAssets = [
  { id: "blockdaemon-ondo-ethereum", name: "Ondo", symbol: "ONDO", price: 0.37195, balance: 0, change24h: 0, image: "", addresses: [{ networkChainId: "ethereum-1", address: "0xfAbA6f8e4a5E8Ab82F62fe7C39859FA577269BE3", decimals: 18 }], tradableSessions: [], priceHistory24h: [], updatedAt: ondoMarketUpdatedAt },
];
await context.route("**/api/ondo-markets", async (route) => {
  await route.fulfill({
    ...
    contentType: "application/json",
    headers: { "Cache-Control": "no-store" },
    body: JSON.stringify({ assets: ondoMarketAssets, configured: true, status: "partial", provider: "blockdaemon", source: "Blockdaemon Token Price API", updatedAt: ondoMarketUpdatedAt }),
  });
});
...
await trustOndoSearchInput.fill("ONDO");
await page.locator('[data-testid="trust-market-search-row-ondo"]').waitFor();
...
await page.locator('[data-testid="trust-market-row-ondo"]').waitFor();
const trustOndoSource = page.locator('[data-testid="trust-ondo-market-source"]');
await trustOndoSource.getByText("Blockdaemon", { exact: false }).waitFor();
await trustOndoSource.getByText("Limited", { exact: true }).waitFor();
...
if (!(await trustOndoRows.evaluateAll((rows) => rows.every((row) => row.getAttribute("data-source") === "Blockdaemon Token Price API")))) {
  throw new Error("Trust Ondo rows are missing their live source attribution.");
}
await page.locator('[data-testid="trust-market-row-ondo"]').getByText("Live price", { exact: true }).waitFor();
const trustOndoPeriod = page.locator('[data-testid="trust-market-period-filter"]');
if (await trustOndoPeriod.inputValue() !== "24h" || await trustOndoPeriod.locator("option").count() !== 1 || await trustOndoPeriod.locator("option").textContent() !== "Spot") {
  throw new Error("Trust Blockdaemon markets should expose a spot-price label instead of an unsupported change period.");
}
const trustOndoNetwork = page.locator('[data-testid="trust-market-network-filter"]');
await trustOndoNetwork.selectOption("ethereum");
await page.locator('[data-testid="trust-market-row-ondo"]').waitFor();
if (await trustOndoRows.count() !== 1) throw new Error("Trust Ondo Ethereum filter did not use source contract metadata.");
await trustOndoNetwork.selectOption("all");
await page.locator('[data-testid="trust-market-row-ondo"]').click();
const trustOndoDetail = page.locator('[data-testid="trust-ondo-market-detail"]');
await trustOndoDetail.getByRole("heading", { name: "Ondo", exact: true }).waitFor();
await trustOndoDetail.getByRole("alert").filter({ hasText: "Live chart data is unavailable." }).waitFor();
await trustOndoDetail.getByText("Market cap, volume, holders, and 24-hour history are not supplied", { exact: false }).waitFor();
