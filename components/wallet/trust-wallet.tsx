...
export function TrustWallet() {
  ...
  const [messages, setMessages] = useState<string[]>(["Ask about this internal demo portfolio. I never request recovery phrases or private keys."]);
  ...
  async function performBuySell() {
    ...
    const record: WalletActivity = { id: requestId, type: buyMode === "buy" ? "receive" : "send", tokenSymbol: buyToken.symbol, amount: tokenAmount, counterpartyLabel: `${buyMode === "buy" ? "Demo purchase" : "Demo sale"} · ${cash(serviceFee, profile.currency)} fee`, date, status: "completed", note: "INTERNAL DEMO" };
    operationLock.current = true;
    setBusy(true);
    try {
      const ok = await commit(next, [record], `${buyMode === "buy" ? "Added" : "Sold"} ${amount(tokenAmount)} ${buyToken.symbol} in the internal demo.`, requestId);
      if (ok) {
        setBuyStage("success");
    ...
    return <div data-testid="trust-home" className="pb-8">
      <header className="flex items-center justify-between gap-2">
        <button type="button" aria-label="Open wallet accounts" onClick={runtime.openAccounts} className="flex min-h-12 min-w-0 items-center gap-2 rounded-full border border-white/[.08] bg-[#181925] px-3 text-left">
          ...
          <span className="min-w-0"><strong className="block truncate text-sm">{activeProfile.walletName}</strong><span className="block truncate text-[10px] font-bold uppercase tracking-[.045em] text-white/38">Larpz Wallet · Demo</span></span>
      ...
      <div data-testid="trust-asset-receive" className="pb-8">
        ...
        {address ? (
          <>
            ...
            <div className="mt-6 rounded-[1.2rem] border border-[#f59e0b]/25 bg-[#f59e0b]/10 p-4 text-sm leading-6 text-white/65">Only send internal {networkName(selected.symbol)} assets to this demo address. No real blockchain funds are supported.</div>
  ...
  function renderSearch() {
    ...
    const features: { label: string; description: string; keywords: string; icon: LucideIcon; action: () => void }[] = [
      ...
      { label: "Swap", description: "Exchange demo tokens", keywords: "trade convert", icon: RefreshCw, action: () => openSwapFor(tokens[0]?.symbol ?? "SOL") },
