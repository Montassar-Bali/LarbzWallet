...
function Action({ label, icon: Icon, onClick, primary = false }: { label: string; icon: LucideIcon; onClick: () => void; primary?: boolean }) {
  return <button type="button" onClick={onClick} className={`flex min-h-[6.3rem] min-w-0 flex-col items-center justify-center gap-2 rounded-[1.35rem] text-[16px]/[20px] font-extrabold active:scale-[.97] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#665cff] ${primary ? "bg-[#4437ff]" : "bg-[#191a28]"}`}><Icon className="size-7" />{label}</button>;
}
