...
import { getTokens, getWalletTheme, saveToken, type WalletToken } from "@/lib/wallet";
    ...
    <main className="wallet-app min-h-screen bg-[#2638d9] text-white" style={{ background: "linear-gradient(180deg, #151e82 0%, #2738d9 28%, #293ce1 100%)" }}>
      <div className="mx-auto min-h-screen w-full max-w-[520px] overflow-hidden bg-[#202fd0] shadow-2xl shadow-black/30">
        ...
        <section className="px-5 pb-24 pt-6">
          ...
          </> : <div className="rounded-3xl bg-black/15 p-6 text-center"><div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-white/15"><SwapHorizontal className="h-7 w-7" /></div><h2 className="text-xl font-bold">{tab === "trade" ? "Trade preview" : "Explore the market"}</h2><p className="mt-2 text-sm leading-6 text-white/65">This demo wallet keeps every action simulated. Choose an action from the button below.</p></div>}
        </section>
        ...
        {actionsOpen && <div className="fixed bottom-24 right-[calc(50%-226px)] z-20 flex flex-col items-end gap-2 sm:right-8">{(["send", "receive", "buy", "trade"] as Action[]).map((action) => <button key={action} type="button" onClick={() => selectAction(action)} className="flex items-center gap-3 text-sm font-semibold"><span className="rounded-full bg-[#1826a9] px-4 py-2 shadow-lg">{action[0].toUpperCase()}{action.slice(1)}</span><span className="grid h-10 w-10 place-items-center rounded-full bg-[#c2caff] text-[#2638d9]">{action === "send" ? <ArrowUpRight className="h-4 w-4" /> : action === "receive" ? <ArrowDownLeft className="h-4 w-4" /> : action === "buy" ? <Plus className="h-4 w-4" /> : <SwapHorizontal className="h-4 w-4" />}</span></button>)}</div>}
